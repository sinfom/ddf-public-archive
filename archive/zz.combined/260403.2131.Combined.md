...
Generated: 2026-04-03 21:31:19.668284
...
DEPENDS: representation (Dirac spinor), choice of D, --, :, Additional fields modify coefficient, dimension (4D used)
---
...
DEPENDS: wavevector., --, ✔ may be cone-compatible (locally or band-limited), ⚠ requires further admissibility constraints, Status:, k, wave packets spread over time
---
...
DEPENDS: QM-EXT-002, 02_operator_notes (Dirac factorisation, spin), 01_foundations F1–F4
---
...
DEPENDS: QM-EXT-002, 04-propagation_rigidity, N8, dirac_factorisation
---
...
TITLE: Ddf p3.5
DEPENDS: the current observable state ( \psi )., [, representing the realised action of (L) under state-dependent constraints., -----, the current observable state., ], No physical laws or constants are derived in this paper., L_\psi, We denote this as:
---
...
TITLE: Ddf p4
DEPENDS: -----, the operator (L^\dagger L) remains well-defined, ( \psi ), feedback influences admissible states and solutions, However:
---
...
DEPENDS: its own state, 2. **Self-regulation**, > the source does not change — the **environment changes how it appears**, the current state of (U), 4. **Emergent variation**, -----, So:, differences arise from response, not multiple sources, observed behaviour = result of their combination, the solution must satisfy both simultaneously., all behaviour comes from one source, the state,, the state is determined by the operator,, 3. **Single origin**, system moves toward balance
---
