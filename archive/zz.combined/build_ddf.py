...
def build_combined(files):
    ...
    def write_output(path):
        ...
        with open(path, "w", encoding="utf-8") as out:
            ...
            for f in files:
                ...
                note_id, title, depends = extract_metadata(f)
    ...
    write_output(COMBINED_CURRENT)
    write_output(COMBINED_ARCHIVE)
...
def build_ai_context_index(files):
    ...
    with open(out_file, "w", encoding="utf-8") as out:
        ...
        for f in files:
            ...
            note_id, title, _ = extract_metadata(f)
...
def build_dependency_graph(files):
    ...
    for f in files:
        note_id, _, _ = extract_metadata(f)
        if note_id:
...
def build_folder_indexes(files):
    ...
    for folder, folder_files in folder_map.items():
        ...
        with open(out_file, "w", encoding="utf-8") as out:
            ...
            sorted_files = sorted(folder_files, key=lambda x: extract_metadata(x)[0] or "")
